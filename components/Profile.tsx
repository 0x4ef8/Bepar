...
import { CheckBadgeIcon, PencilIcon, WalletIcon, ClockIcon, ShieldExclamationIcon, ExclamationCircleIcon, StarIcon, CalendarIcon } from './Icons';
...
const Profile: React.FC<ProfileProps> = ({ onPostListing }) => {
    ...
    const currentUserData = useMemo(() => users.find(u => u._id === user?._id), [users, user]);
        ...
        <div className="container mx-auto p-4 sm:p-6 lg:p-8">
            ...
            <div className="bg-white rounded-xl shadow-lg p-8 mb-8">
                <div className="flex flex-col sm:flex-row items-center sm:items-start text-center sm:text-left gap-8">
                    <img className="w-32 h-32 rounded-full ring-4 ring-primary ring-offset-2" src={currentUserData.photo} alt={currentUserFullName} />
                    <div>
                        <div className="flex items-center justify-center sm:justify-start gap-3">
                           <h2 className="text-3xl font-bold text-gray-800">{currentUserFullName}</h2>
                           <KycStatusBadge status={currentUserData.kycStatus} />
                        </div>
                        <p className="text-gray-600 mt-1">{currentUserData.phone}</p>
                        <p className="text-gray-600">{currentUserData.email || 'No email provided.'}</p>
                        ...
                        <div className="mt-4 flex flex-wrap gap-4 justify-center sm:justify-start">
                           <div className="text-center p-2 rounded-lg bg-gray-50 flex-grow">
                               ...
                               <p className="text-sm text-gray-500">Rating ({currentUserData.reviews?.length || 0})</p>
                           </div>
                            <div className="text-center p-2 rounded-lg bg-gray-50 flex-grow">
                            ...
                            <div className="text-center p-2 rounded-lg bg-gray-50 flex-grow">
                               <p className="font-bold text-xl">रू{currentUserData.walletBalance.toLocaleString('en-NP')}</p>
                               <p className="text-sm text-gray-500 flex items-center justify-center gap-1"><WalletIcon className="w-4 h-4"/> Wallet</p>
                           </div>
                        </div>
                    </div>
                     <button onClick={() => setEditModalOpen(true)} className="sm:ml-auto flex items-center gap-2 bg-gray-100 hover:bg-gray-200 text-gray-700 font-medium py-2 px-4 rounded-lg transition-colors">
                ...
                <div className="lg:col-span-2">
                    ...
                    {userListings.length > 0 ? (
                        ...
                        <div className="text-center py-16 bg-white rounded-lg shadow">
                            ...
                            <button onClick={onPostListing} className="mt-4 bg-primary text-white font-semibold px-6 py-2 rounded-full hover:bg-blue-700 transition-colors">
                ...
                <div className="lg:col-span-1">
                    <h3 className="text-2xl font-bold text-gray-800 mb-6">My Reviews</h3>
                     <div className="bg-white rounded-xl shadow-lg p-6 space-y-4">
                        {(currentUserData.reviews && currentUserData.reviews.length > 0) ? currentUserData.reviews.map(review => {
                            ...
                            <p className="text-center text-gray-500 py-8">You have not received any reviews yet.</p>
                        )}
                    </div>
                </div>
            </div>
